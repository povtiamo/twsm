	...
	try:
		with open(luaFile,'w') as luafile:
			with open(jsonFile) as jsonfile:
				ret = str_to_lua_table(jsonfile.read())
...
fileList = os.popen("find ./ -name \"*.json\" " ).read().split("\n")
for path in fileList:
	if "" == path :
		continue
	luaPath = path.replace(".json", ".lua")
	file_to_lua_file(path, luaPath)


file_to_lua_file("./a.json", "./a.lua")
