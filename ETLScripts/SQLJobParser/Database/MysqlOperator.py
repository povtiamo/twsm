...
class MysqlOperator(DBOperator):
    ...
    def __init__(self,database_cat,database_str,logger):
        ...
        engine = create_engine(self.engine_info,max_overflow=5)  #初始化数据库连接 echo=True
        DBsesion = sessionmaker(bind=engine) #创建DBSession类型
        self.session = DBsesion() #建session对象
        self.logger = logger
        self.logger.info(u'Mysql 连接成功。')
...
if __name__ == '__main__' :
    ...
    logger = CustomLogger(sql_file_path,'2018-10-10','410000')

    pgsql_oper=MysqlOperator('MYSQL','mysql_for_metadata',logger)
